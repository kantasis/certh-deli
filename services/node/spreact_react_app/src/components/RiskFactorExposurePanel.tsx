...
const riskFactorExposurePanel: React.FC = () => {
    ...
    useEffect(() => {
        ...
        console.log("Restoring URL params:");
        for (const [key, value] of params.entries()) {
            console.log("PAOK: " + key, value);
        }
    ...
    useEffect(() => {
        ...
        if ([0, 1, 2, 3, 4, 5].includes(rfVal)) {
            set_selectedAnalysis(1); // Nutrition
        } else {
            set_selectedAnalysis(2); // Lifestyle
        }
    ...
    const buildIframeUrl = (panelId: number, isDiet: boolean = false) => {
        ...
        const sex = sex_dictLst[selectedSex_int].var_filter;
        const age = age_dictLst[selectedAge_int].var_filter;
        const crc = crcFactors_dictLst[selectedCrcFactors_int].var_filter;
        const params = [
            countryParams,
            yearParams,
        ...
        if (isDiet) {
            ...
            params.push(`var-diet_type_filter=${encodeURIComponent(dietType)}`);
        } else {
            const rf = riskFactorExposure_dictLst[selectedRiskFactorExposure_int].var_filter;
            params.push(`var-riskFactor_filter=${encodeURIComponent(rf)}`);
        }

        return `${grafana_url}&panelId=${panelId}&${params.filter(Boolean).join("&")}`;
    };

    const iframeContent = selectedAnalysis_int === 1
        ? (<iframe src={buildIframeUrl(5, true)} width="100%" height="600px" />)
        : (<iframe src={buildIframeUrl(8, false)} width="100%" height="600px" />);
    ...
    return (
        <div className="row">
            ...
            <div className="col-sm-2 mt-3">
                ...
                />
                <CountryFilter selectedCountries_lst={selectedCountries_lst} set_selectedCountries={set_selectedCountries} />
                <YearFilter minYear_int={minYear_int} set_minYear={set_minYear} maxYear_int={maxYear_int} set_maxYear={set_maxYear} />
            ...
            <div className="col-sm-8 mt-5">
                ...
                <SaveGraphButton iframeUrl={selectedAnalysis_int === 1 ? buildIframeUrl(5, true) : buildIframeUrl(8, false)} />
